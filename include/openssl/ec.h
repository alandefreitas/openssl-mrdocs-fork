...
/**
 * @brief Select whether EC parameters are encoded as a named curve or explicitly.
 * @param ctx EVP_PKEY context for EC parameter or key generation.
 * @param param_enc OPENSSL_EC_NAMED_CURVE or OPENSSL_EC_EXPLICIT_CURVE.
 * @return 1 on success, or a negative value on failure.
 */
...
/**
 * @brief Set the ECDH key-derivation output length for a key-exchange context.
 * @param ctx EVP_PKEY context configured for ECDH with a KDF.
 * @param len Desired derived-key length in bytes.
 * @return 1 on success, or a negative value on failure.
 */
...
#ifndef OPENSSL_NO_DEPRECATED_3_0
/**
 * @brief Opaque description of the field arithmetic and curve methods for an EC_GROUP.
 *
 * Concrete EC_METHOD objects (for example EC_GFp_simple_method()) select how
 * group and point operations are implemented for a given field type.
 */
...
/**
 * @brief Opaque elliptic-curve point (coordinates over the field of an EC_GROUP).
 */
...
/**
 * @brief Return whether an EC_GROUP encodes as a named curve or with explicit parameters.
 * @param group EC_GROUP whose ASN.1 encoding flag is queried.
 * @return OPENSSL_EC_NAMED_CURVE or OPENSSL_EC_EXPLICIT_CURVE (0 on older APIs).
 */
...
#ifndef OPENSSL_NO_DEPRECATED_3_0
/**
 * @brief Encode an EC point as a BIGNUM holding the SEC1 octet form (deprecated).
 * @param group Curve that defines @p point.
 * @param point Point to encode.
 * @param form Conversion form passed to EC_POINT_point2oct().
 * @param bn Optional BIGNUM to reuse for the result, or NULL to allocate one.
 * @param ctx BN_CTX for temporary values, or NULL.
 * @return Pointer to the result BIGNUM (same as @p bn when non-NULL), or NULL on error.
 */
OSSL_DEPRECATEDIN_3_0 BIGNUM *EC_POINT_point2bn(const EC_GROUP *group,
    const EC_POINT *point,
    point_conversion_form_t form,
    BIGNUM *bn, BN_CTX *ctx);
/**
 * @brief Decode an EC point from a BIGNUM holding the SEC1 octet form (deprecated).
 * @param group Curve that defines the point encoding.
 * @param bn BIGNUM whose big-endian content is the SEC1 octet string of the point.
 * @param point Optional EC_POINT to reuse for the result, or NULL to allocate one.
 * @param ctx BN_CTX for temporary values, or NULL.
 * @return Pointer to the result EC_POINT (same as @p point when non-NULL), or NULL on error.
 */
OSSL_DEPRECATEDIN_3_0 EC_POINT *EC_POINT_bn2point(const EC_GROUP *group,
    const BIGNUM *bn,
 ...
 * @return Pointer to the static ASN1_ITEM for ECPKPARAMETERS.
 */
...
/**
 * Return the NID of the basis type used to represent field elements
 * of @p group (characteristic-2 curves).
 * @param group Elliptic-curve group.
 * @return Basis-type NID, or 0 if not applicable / on error.
 */
...
#ifndef OPENSSL_NO_DEPRECATED_3_0
/**
 * @brief Print EC domain parameters from an EC_GROUP to a BIO (deprecated).
 * @param bp BIO that receives the human-readable parameters.
 * @param x EC group whose parameters are printed.
...
#ifndef OPENSSL_NO_DEPRECATED_3_0
/**
 * @brief Create a new empty EC_KEY using a library context (deprecated).
 * @param ctx Library context for algorithm fetches, or NULL for the default.
 * @param propq Optional property query string for provider selection, or NULL.
 * @return New EC_KEY, or NULL if an error occurred.
 */
...
/**
 * @brief Derive key material from an ECDH shared secret using the X9.63 KDF (deprecated).
 *
 * Historically named for ANSI X9.62; the KDF is actually specified in ANSI X9.63.
 * Retained for ABI compatibility (prefer X9.63-named APIs).
 *
 * @param out Destination buffer for the derived key material.
 * @param outlen Number of bytes to write to @p out.
 * @param Z Shared secret (ECDH Z) octets used as KDF key input.
 * @param Zlen Length of @p Z in bytes.
 * @param sinfo Optional shared-info / OtherInfo octets bound into the KDF, or NULL if @p sinfolen is 0.
 * @param sinfolen Length of @p sinfo in bytes.
 * @param md Message digest used by the X9.63 KDF (for example SHA-256).
 * @return 1 on success, or 0 on failure.
 */
...
/**
 * @brief Decode an ECDSA signature from DER.
 * @param a Optional destination pointer updated to the result, or NULL.
 * @param in Address of a pointer to the DER input; advanced past the decoded value.
 * @param len Number of bytes available at *@p in.
 * @return Decoded ECDSA_SIG, or NULL on error.
 */
ECDSA_SIG *d2i_ECDSA_SIG(ECDSA_SIG **a, const unsigned char **in, long len);
/**
 * @brief Encode an ECDSA signature (r, s) to DER.
...
/**
 * @brief Borrow pointers to the r and s components of an ECDSA signature.
...
/**
 * @brief Return a borrowed pointer to the r component of an ECDSA signature.
 * @param sig Signature to query.
 * @return Internal pointer to r, or NULL if unset; do not free.
 */
const BIGNUM *ECDSA_SIG_get0_r(const ECDSA_SIG *sig);

/**
 * @brief Return a borrowed pointer to the s component of an ECDSA signature.
 * @param sig Signature to query.
 * @return Internal pointer to s, or NULL if unset; do not free.
 */
const BIGNUM *ECDSA_SIG_get0_s(const ECDSA_SIG *sig);

/**
 * @brief Set the r and s components of an ECDSA signature, transferring ownership.
...
/**
 * Allocate a new EC_KEY_METHOD, optionally copying @p meth.
 * @param meth Method to copy, or NULL for an empty method table.
 * @return A newly allocated method table, or NULL on error.
 */
OSSL_DEPRECATEDIN_3_0 EC_KEY_METHOD *EC_KEY_METHOD_new(const EC_KEY_METHOD *meth);
/**
 * Free an EC_KEY_METHOD.
 * @param meth Method table to free; may be NULL.
 */
OSSL_DEPRECATEDIN_3_0 void EC_KEY_METHOD_free(EC_KEY_METHOD *meth);
/**
 * @brief Install lifecycle and field-assignment callbacks on an EC_KEY_METHOD (deprecated).
 * @param meth Method table to update.
 * @param init Optional constructor called when an EC_KEY using @p meth is created.
 * @param finish Optional destructor called when such an EC_KEY is freed.
 * @param copy Optional deep-copy callback from @p src into @p dest.
 * @param set_group Optional callback to assign the EC_GROUP on the key.
 * @param set_private Optional callback to assign the private BIGNUM.
 * @param set_public Optional callback to assign the public EC_POINT.
 */
...
/**
 * @brief Install ECDSA signing callbacks on an EC_KEY_METHOD.
 * @param meth Method object to update.
 * @param sign Optional ECDSA_sign-style callback (type, digest, signature buffer).
 * @param sign_setup Optional precomputation of kinv/r for signing.
 * @param sign_sig Optional callback that returns an ECDSA_SIG structure.
 *
