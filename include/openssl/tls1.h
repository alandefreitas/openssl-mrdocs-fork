...
/**
 * @brief Set the maximum fragment length mode for an SSL connection.
 * @param ssl SSL connection to configure.
 * @param mode TLSEXT_max_fragment_length_* mode value.
...
/**
 * @brief Return the server name indication (SNI) hostname associated with an SSL connection.
 * @param s SSL connection to query.
 * @param type Name type; currently only TLSEXT_NAMETYPE_host_name is supported.
 * @return Hostname string, or NULL if none is present for @p type.
 */
__owur const char *SSL_get_servername(const SSL *s, const int type);
/**
 * @brief Return the type of the server name indication currently associated with a connection.
 * @param s SSL connection to query.
 * @return TLSEXT_NAMETYPE_host_name if an SNI hostname is present, or -1 if none.
 */
__owur int SSL_get_servername_type(const SSL *s);
/**
 * @brief Export keying material derived from the connection's exporter secret.
 * @param s SSL connection to export from.
 * @param out Buffer that receives @p olen bytes of keying material.
 * @param olen Number of bytes to write to @p out.
 * @param label Application-specific label.
 * @param llen Length of @p label in bytes.
 * @param context Optional application context bytes.
 * @param contextlen Length of @p context in bytes.
 * @param use_context Nonzero to include @p context; zero to omit it.
 * @return 1 on success, or 0 or -1 on failure.
 */
...
/**
 * @brief Export keying material derived from the early exporter master secret.
 * @param s SSL connection to export from.
 * @param out Buffer that receives @p olen bytes of keying material.
 * @param olen Number of bytes to write to @p out.
 * @param label Application-specific label.
...
/**
 * @brief Get the NID of the signature type the peer used to sign TLS handshake messages.
 * @param s SSL connection that has verified a peer signature (typically TLSv1.2 or later).
 * @param pnid Receives the signature-type NID (public-key type, or EVP_PKEY_RSA_PSS for PSS).
 * @return 1 on success, or 0 on failure (for example no peer signature, version below 1.2, or called too early).
 */
int SSL_get_peer_signature_type_nid(const SSL *s, int *pnid);
/**
 * @brief Get the NID of the signature type used locally to sign TLS handshake messages.
 * @param s SSL connection that has performed a signature (typically TLSv1.2 or later).
 * @param pnid Receives the signature-type NID (public-key type, or EVP_PKEY_RSA_PSS for PSS).
 * @return 1 on success, or 0 on failure (for example no signature, version below 1.2, or called too early).
 */
int SSL_get_signature_type_nid(const SSL *s, int *pnid);

/**
 * @brief Get a peer-supported signature algorithm by index.
 * @param s SSL connection to query.
 * @param idx Zero-based index into the peer's signature algorithm list.
 ...
 * @return Number of signature algorithms, or 0 if @p idx is out of range.
 */
