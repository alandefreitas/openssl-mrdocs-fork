...
/**
 * @brief Return how many bytes the peer half of a BIO pair last tried to read while the buffer was empty.
 * @param b BIO-pair endpoint to query.
 * @return Requested byte count (capped by the write guarantee), or 0 after a successful read or once the request is satisfied.
 */
...
/**
 * @brief Report whether a datagram socket errno / WSA error is a non-fatal retryable condition.
 * @param error Platform socket error code (for example EAGAIN, EINTR, or WSAEWOULDBLOCK).
 * @return 1 if datagram BIO I/O should retry, or 0 if the error is fatal.
 */
...
/**
 * @brief Wait on a socket until it is ready for I/O or until @p max_time elapses.
 * @param fd Socket file descriptor to poll.
 * @param for_read Non-zero to wait for readability; zero to wait for writability.
 * @param max_time Absolute deadline as a time_t; 0 means no timeout (succeed immediately).
 * @return 1 on success, 0 on timeout, or -1 on error.
 */
...
/**
 * @brief Split a host:service string into separately allocated host and service names.
 * @param hostserv Combined host and/or service string (IPv6 hosts must be bracketed).
 * @param host Receives a malloc'd hostname string, or is left unchanged / set to NULL per the parse rules; free with OPENSSL_free().
 * @param service Receives a malloc'd service/port string, or is left unchanged / set to NULL per the parse rules; free with OPENSSL_free().
 * @param hostserv_prio BIO_PARSE_PRIO_HOST or BIO_PARSE_PRIO_SERV when @p hostserv has no colon.
 * @return 1 on success, or 0 on error.
 */
...
/**
 * @brief Install the multi-message send callback used by BIO_sendmmsg() on a BIO_METHOD.
 * @param biom Method table to update.
 * @param f Callback implementing BIO_sendmmsg()-style output; may be NULL.
 * @return 1 on success, or 0 on failure.
 */
