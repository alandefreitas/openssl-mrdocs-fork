...
/**
 * @brief Map a trace category name to its OSSL_TRACE_CATEGORY_* number.
 * @param name Category name such as "TRACE", "TLS", or "ALL".
 * @return Category number, or -1 if @p name is unknown.
 */
int OSSL_trace_get_category_num(const char *name);

/* Returns the trace category name for the given |num| */
/**
 * @brief Map an OSSL_TRACE_CATEGORY_* number to its canonical name string.
 * @param num Category number from OSSL_trace_get_category_num() or a OSSL_TRACE_CATEGORY_* constant.
 * @return Category name (for example "TLS"), or NULL if @p num is unrecognized.
 */
 ...
 */
/**
 * @brief Attach a BIO as the simple trace channel for @p category (ownership transfers on success).
 * @param category OSSL_TRACE_CATEGORY_* value selecting the channel.
 * @param channel Sink BIO for trace output, or NULL to disable tracing for @p category.
 ...
 */
/**
 * @brief Set the text printed before each trace group for @p category.
 * @param category OSSL_TRACE_CATEGORY_* value selecting the channel.
 * @param prefix Prefix string, or NULL to clear any existing prefix.
 * @return 1 on success, or 0 on failure.
 */
int OSSL_trace_set_prefix(int category, const char *prefix);
/**
 * @brief Set the text printed after each trace group for @p category.
 * @param category OSSL_TRACE_CATEGORY_* value selecting the channel.
 * @param suffix Suffix string, or NULL to clear any existing suffix.
 * @return 1 on success, or 0 on failure.
 */
int OSSL_trace_set_suffix(int category, const char *suffix);

/**
 * @brief Application callback that receives OpenSSL trace output for a category.
 * @param buffer Trace text (may span multiple lines); never empty on WRITE.
 * @param count Number of bytes at @p buffer.
 * @param category OSSL_TRACE_CATEGORY_* for this channel.
 * @param cmd One of OSSL_TRACE_CTRL_BEGIN, OSSL_TRACE_CTRL_WRITE, or OSSL_TRACE_CTRL_END.
 * @param data User pointer supplied to OSSL_trace_set_callback().
 * @return Number of bytes written (must be non-zero on success), or 0 on error.
 */
 ...
 */
/**
 * @brief Enable tracing for @p category by sending output to @p callback.
 * @param category OSSL_TRACE_CATEGORY_* value selecting the channel.
 * @param callback Trace callback, or NULL to disable tracing for @p category.
 * @param data User pointer passed to @p callback on each invocation.
 * @return 1 on success, or 0 on failure.
 *
 ...
 */
/**
 * @brief Test whether a trace channel is currently attached for @p category.
 * @param category OSSL_TRACE_CATEGORY_* value to query.
 * @return 1 if tracing is enabled for @p category, or 0 otherwise.
 */
 ...
 */
/**
 * @brief Begin a locked trace output group for @p category and return its BIO channel.
 * @param category OSSL_TRACE_CATEGORY_* value selecting the channel.
 * @return Trace BIO to write to, or NULL if tracing is disabled for @p category.
 *
...
/**
 * @brief Write @p data to a trace BIO as text, optionally masking controls and truncating.
 * @param out Destination BIO (typically the channel from OSSL_trace_begin()).
 * @param text Non-zero to emit raw text; zero masks control characters except newlines.
 * @param full Non-zero to emit the full buffer; zero truncates to OSSL_TRACE_STRING_MAX characters.
 * @param data Bytes to print.
 * @param size Number of bytes at @p data.
 * @return Number of characters emitted, or -1 on error.
 */
