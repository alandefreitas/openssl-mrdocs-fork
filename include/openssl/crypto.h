...
/**
 * @brief Encode @p buf as a hexadecimal string into @p str, optionally separated by @p sep.
 * @param str Destination buffer for the hex text, or NULL to only compute the required length.
 * @param str_n Capacity of @p str in bytes (ignored when @p str is NULL).
 * @param strlength Receives the result length (not counting a trailing NUL), or may be NULL.
 * @param buf Bytes to encode.
 * @param buflen Number of bytes at @p buf.
 * @param sep Separator inserted between octets, or '\0' for no separator.
 * @return 1 on success, or 0 on failure (for example if @p str_n is too small).
 */
...
/**
 * @brief Allocate ex_data at index @p idx by invoking the registered new callback if unset.
 * @param class_index CRYPTO_EX_INDEX_* identifying the object class.
 * @param obj Object that owns @p ad (passed to the new callback).
 * @param ad Ex-data table to allocate into.
 * @param idx Index from CRYPTO_get_ex_new_index() (or a class helper).
 * @return 1 on success (including when a value is already present), or 0 on failure.
 */
int CRYPTO_alloc_ex_data(int class_index, void *obj, CRYPTO_EX_DATA *ad,
    int idx);

/**
 * @brief Store @p val at ex_data index @p idx, replacing any previous pointer.
 * @param ad Ex-data bag to update.
 * @param idx Index from CRYPTO_get_ex_new_index() (or a class helper).
 * @param val Pointer to store (may be NULL to clear the slot).
 * @return 1 on success, or 0 on failure.
 */
...
/**
 * @brief Report whether the process appears to be a non-interactive Windows service.
 * @return 1 if running as a service (Windows), 0 if interactive / not a service, or -1 on query failure; always 0 on non-Windows.
 */
